    ...
    class ImageFilesRepository {
        ...
        static storeImageData(previousGUID, imageDataBase64) {
            if (imageDataBase64) {
                ...
                /*
                var Jimp = require('jimp'); // does not support webp format
                Jimp.read(this.getServerImageFileURL(GUID), (err, original) => {
                    if (err) throw err;
                    original
                        .resize(newWidth, newHeight) // resize
                        .write(this.getServerThumbnailFileURL(GUID)); // save
                });*/
                return GUID;
            } else
                return previousGUID;
